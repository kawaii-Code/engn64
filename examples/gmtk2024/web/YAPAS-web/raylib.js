...
class RaylibJs {
    ...
    draw_all_pixels_to_the_canvas_through_the_madness_of_wasm_and_js(pixels_ptr) {
        const buffer = this.wasm.instance.exports.memory.buffer;
        const pixels = new Uint32Array(buffer, pixels_ptr, 64 * 64);
        for (let y = 0; y < 64; y++) {
            for (let x = 0; x < 64; x++) {
                const color = color_hex(pixels[y * 64 + x]);
    ...
    async start({ wasmPath, canvasId }) {
        if (this.wasm !== undefined) {
            alert("The game is already running. Please stop() it first.");
            return;
        }
