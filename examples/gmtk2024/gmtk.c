...
bool8 is_rect_on_ground(Rect_Int8 rect, Level *level) {
    ...
    for (uint32 i = 0; i < level->boxes.length; i++) {
        Box *box = &level->boxes.items[i];
        ...
        for (int8 x = 0; x < TILEMAP_WIDTH; x++) {            
            ...
            switch (c) {
                ...
                case 'd': {
                    ...
                    door_rect.y -= 4;
                    door_rect.height += 4;
                    dynamic_array_append(&level.doors, ((Door) {
                        .rect = door_rect,
                    }));
                } break;

                case 'b': {
                    dynamic_array_append(&level.boxes, ((Box) {
                        .rect = tile_rect,
                    }));
                } break;
                ...
                case 'c': {
                    ...
                    dynamic_array_append(&level.candies, ((Candy) {
                        .rect = candy_rect,
                    }));
                } break;
                ...
                case 'k': {
                    ...
                    key_rect.y += 1;
                    dynamic_array_append(&level.keys, key_rect);
                } break;
            }
        }
    }
...
void reload_current_level(Game *game) {
    ...
    Level level = original_level;
    level.boxes = (Dynamic_Array_Of_Boxes){0};
    level.candies = (Dynamic_Array_Of_Candies){0};
    level.doors = (Dynamic_Array_Of_Doors){0};
    level.keys = (Dynamic_Array_Of_Rects){0};

    for (uint32 i = 0; i < original_level.boxes.length; i++) {
        dynamic_array_append(&level.boxes, original_level.boxes.items[i]);
    }
    for (uint32 i = 0; i < original_level.candies.length; i++) {
        dynamic_array_append(&level.candies, original_level.candies.items[i]);
    }
    for (uint32 i = 0; i < original_level.doors.length; i++) {
        dynamic_array_append(&level.doors, original_level.doors.items[i]);
    }
    for (uint32 i = 0; i < original_level.keys.length; i++) {
        dynamic_array_append(&level.keys, original_level.keys.items[i]);
    }
...
// TODO:
// Sounds
// - Jump, Land, Key pickup, Next level, Candy eat, Scaling, Step?
// Maybe little particle effects
// WebGL
...
void update_and_draw(Game *game, Screen *screen, Input *input, float32 dt) {
    ...
    Dynamic_Array_Of_Boxes *boxes = &current_level->boxes;
    Dynamic_Array_Of_Candies *candies = &current_level->candies;
    Dynamic_Array_Of_Doors *doors = &current_level->doors;
    Dynamic_Array_Of_Rects *keys = &current_level->keys;

    if (input->btn2.just_pressed) {
        reload_current_level(game);
        PlaySound(game->sounds.retry);
    }
    if (input->btn3.just_pressed) {
    ...
    {
        // Trigger collisions
        for (uint32 i = 0; i < candies->length; i++) {
            Candy *candy = &candies->items[i];
            if (rect_intersect_rect_int8(player->rect, candy->rect)) {
                //PlaySound(game->sounds.shot);
                dynamic_array_swap_remove(candies, i);
            ...
            if (!result.has_conflict) {
                ...
                for (int i = 0; i < result.conflicts_count; i++) {
                    ...
                    while (conflict.other == ENTITY_BOX) {
                        Box *box = &boxes->items[conflict.param];
        ...
        *player_was_falling_last_frame = !player_is_on_ground;
        ...
        for (uint32 i = 0; i < keys->length; i++) {
            Rect_Int8 rect = keys->items[i];
            if (rect_intersect_rect_int8(player->rect, rect)) {
                PlaySound(game->sounds.pickup);
                dynamic_array_swap_remove(keys, i);
                player->has_key = true;
            }
        }

        for (uint32 i = 0; i < doors->length; i++) {
            Door *door = &doors->items[i];
            if (rect_intersect_rect_int8(player->rect, door->rect)) {
                if (player->has_key) {
                    if (game->current_level_index < LEVELS_COUNT) {
                        PlaySound(game->sounds.next_level);
                        game->current_level_index += 1;
    ...
    // Update boxes
    {
        for (uint32 i = 0; i < boxes->length; i++) {
            Box *box = &boxes->items[i];
